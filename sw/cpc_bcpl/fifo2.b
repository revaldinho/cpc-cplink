...
AND fifo_out_bytes_nc(txptr, max) = VALOF $(
  LET f=0
...
AND start() = VALOF $(
  ...
  writef("Test 2: Sending and Receiving 255 byte blocks, no status checks*n")
  i:=0
  j:=0
  resettime()
  WHILE i < DBL_SZ DO i:= i+ (fifo_out_bytes_nc( txbyteptr+i, DBL_SZ-i) )
  WHILE j < DBL_SZ DO j:= j+ (fifo_in_bytes_nc( rxbyteptr+j, DBL_SZ-j) )
  t := scaledtime(2)         // ask for time in 75th of sec.
