...
REM Simple byte-at-a-time loopback
REPEAT
    IF ((wptr%+1) MOD QUEUE_SZ% <> rptr%) AND FN_gpio_get(gpio%, PIN_DOR%))<>0 THEN
       queue%(wptr%)=FN_read_fifo_byte()
       wptr%=(wptr%+1) MOD QUEUE_SZ%
    ENDIF
    IF (wptr%<>rptr% AND FN_gpio_get(gpio%, PIN_DIR%)<>0) THEN
        PROC_write_fifo_byte(queue%(rptr%))
        rptr%=(rptr%+1) MOD QUEUE_SZ%
    ENDIF            
UNTIL 0
END
...
DEF PROC_write_fifo_byte( txdata% )
    ...
    FOR b%=0 TO 7
        ...
        mask%= mask%<<1
        PROC_gpio_out(PIN_DATA%(b%))
    NEXT b%

    PROCgpio_set(gpio%,data%+PMASK_WNR%)
    PROCgpio_set(gpio%,data%+PMASK_WNR%+PMASK_SI%)
    PROCgpio_clr(gpio%,PMASK_SI%)
...
DEF FN_read_fifo_byte()
    ...
    REM            _______
    REM   DATA ---X_@_____X-----  @=strobe point
    REM               __
