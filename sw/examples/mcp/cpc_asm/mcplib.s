...
;; ***************************
    ...
    jp InitCPLink                           ;; &8000 Initialise the cplink interface
    jp SendTextCommand                      ;; &8003 send a text command (wrapped in a packet) via cplink to the pi
    jp ReceiveTextCommandResponse           ;; &8006 receive a response from a text command (wrapped in a packet) via cplink from the pi
    jp SendBinaryCommand                    ;; &8009 send a binary command (wrapped in a packet) via cplink to the pi
    jp ReceiveBinaryCommandResponse         ;; &800C receive a response from a binary command (wrapped in a packet) via cplink from the pi
    jp SendBinaryData                       ;; &800F send a BinaryData payload (wrapped in a packet) via cplink to the pi
    jp ReceiveBinaryData                    ;; &8012 receive a BinaryData payload (wrapped in a packet) via cplink from the pi
...
;; ********************************
;; Functions called from Jump Block
;; ********************************
...
InitCPLink
	call _fifo_reset	;; reset the fifo queue
    ret
...
;;      call &8003,&buffer,bufferlength%,@error%
;;          where buffer is a memory address holding the command and its params to send (IX 4,5)
;;          where bufferlength is the integer size of the buffer to send (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
;;
...
;; *************************************
;; Receive a TextCommand response via 
;; CP-LINK from pi and optional print it
;;
;; Entry
;; - called from basic as 
;;      call &8006,&buffer,@bufferlength%,printval,%@error%
;;          where buffer is a memory address holding the command and its params to send (IX 6,7)
;;          where bufferlength is the integer size of the packet body received (IX 4,5)
;;          where printval is 0 or 1.  1 will print arriving characters in the response (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
;;
;; - A will have number of parameters
;;
...
;; Entry
;; - called from basic as 
;;      call &8009,&buffer,bufferlength%,@error%
;;          where buffer is a memory address holding the command and its params to send (IX 4,5)
;;          where bufferlength is the integer size of the buffer to send (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
;;
...
;; *************************************
;; Receive a BinaryCommand response via 
;; CP-LINK from pi and optional print it
;;
;; Entry
;; - called from basic as 
;;      call &800C,&buffer,@bufferlength%,printval,@error%
;;          where buffer is a memory address holding the command and its params to send (IX 6,7)
;;          where bufferlength is the integer size of the packet body received (IX 4,5)
;;          where printval is 0 or 1.  1 will print arriving characters in the response (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
...
;; Entry
;; - called from basic as 
;;      call &800F,&buffer,bufferlength%,@error%
;;          where buffer is a memory address holding the command and its params to send (IX 4,5)
;;          where bufferlength is the integer size of the buffer to send (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
;;
...
;; Entry
;; - called from basic as 
;;      call &8012,&buffer,@bufferlength%,printval,@error%
;;          where buffer is a memory address holding the command and its params to send (IX 6,7)
;;          where bufferlength is the integer size of the packet body received (IX 4,5)
;;          where printval is 0 or 1.  1 will print arriving characters in the response (IX 2,3)
;;          where error is an integer address so error values can be returned (IX 0,1)
...
NextEndDelimiter
    ...
    ld (hl),0x0                 ;; clear the byte we have writing too (we dont want it)
    ret 
...
;; **********************************************
;; Returns an error to basic variable in call
;;
;; Entry
